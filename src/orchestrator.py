...
def call_function(
    ...
    # close the name field and open "parameters" so the model sees the JSON built so far
    scaffold_ids = model.encode('", "parameters": {')[0].tolist()
...
def _build_instruction_prefix(prompt: str, available_functions: list[FunctionDef]) -> str:
    """Describe the available functions and frame the expected JSON answer."""
    lines = [
        "You are a function-calling assistant. Choose exactly one function that "
        "satisfies the user's request and provide its parameters.",
        "",
        "Available functions:",
    ]
    for func in available_functions:
        param_list = ", ".join(f"{name}: {p_type.type}" for name, p_type in func.parameters.items())
...
def _generate_parameters(
    ...
    """For parameters in function_def, generate a value using appropriate Grammar.
    Returns the parameters dict and the context extended with all generated tokens.
    """
    parameters: dict = {}
    for i, (param_name, param_type) in enumerate(function_def.parameters.items()):
        # tell the model which parameter slot it is filling before generating its value
